...
import { AIUnlockAnimation } from "@/components/motion-core/ai-unlock-animation.tsx";

export function AIUnlockAnimationDemo() {
  return <AIUnlockAnimation />;
}
```
...
```bash
npx shadcn@latest add @uitripled/ai-unlock-animation
```
...
<ComponentSource name="ai-unlock-animation" title="@/components/motion-core/ai-unlock-animation.tsx" />
