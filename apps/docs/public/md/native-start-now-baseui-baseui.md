...
import { NativeStartNow } from "components/native/baseui/native-start-now-baseui.tsx";

export function NativeStartNowDemo() {
  return <NativeStartNow />;
}
```
...
```bash
npx shadcn@latest add @uitripled/native-start-now-baseui-baseui
```
...
The `Native Start Now` component uses the following components. Make sure you have them installed in your project.

- button

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="native-start-now-baseui-baseui" title="components/native/baseui/native-start-now-baseui.tsx" />
