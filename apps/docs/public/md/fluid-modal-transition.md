...
import { FluidModalTransition } from "@/components/motion-core/fluid-modal-transition.tsx";

export function FluidModalTransitionDemo() {
  return <FluidModalTransition />;
}
```
...
```bash
npx shadcn@latest add @uitripled/fluid-modal-transition
```
...
<ComponentSource name="fluid-modal-transition" title="@/components/motion-core/fluid-modal-transition.tsx" />
